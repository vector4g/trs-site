...
## Iteration 26 — 2026-07-07 (Footer glossary + /medication-at-borders + /assistance-codes + catch-22 citation retrofit)
- Footer: Glossary link added (footer-glossary-link) — sitewide crosslink, user-approved.
- NEW routes via ReferenceArticle (which gained `relatedLinks` prop): medication-at-borders (6 H2s, 6 links, byte-identical verified), assistance-codes (6 H2s, 5 links, byte-identical verified). TOP_PAGES, App.js, sitemap (lastmod 2026-07-07), llms.txt ×2 (capitalisation-exact).
- Cross-links: SCD row +2 (medication, assistance); civil-society gained relatedLinks row (medication); new pages link each other.
- catch-22 RETROFIT (no body text changes — verified: md diff is {#id}-suffix-only, 10 lines): 10 heading anchors authored in catch-22.md + BriefSection gained optional `headingId` prop (memo unaffected) + 10 headingIds passed. Anchor list: executive-summary, the-h-m-precedent, the-shadow-hr-pattern-named, the-disclosure-collapse-multiplier, the-enforcement-trajectory, the-architectural-resolution, five-diagnostic-questions, if-this-brief-maps-to-your-organisation, further-reading-the-exposure-series, sources-citations. Cite block under standfirst (catch22-citation-block, "Verified June 2026."). Article JSON-LD dateModified → 2026-07-07 (datePublished 2026-05-01 unchanged). llms.txt /catch-22 line already present (confirmed, unchanged). Sitemap /catch-22 lastmod already 2026-07-07.
- PRODUCTION STATE: previous batch (civil-society, travel-risk-dpia, glossary) verified LIVE. THIS batch verified in preview + build shells; AWAITING NEXT DEPLOY.
...
## Iteration 31 — 2026-07-08 (/security page: Security and data handling)
New route /security (SecurityPage.jsx + content/security.md + ReferenceArticle) per verbatim spec. Two user-approved wording amendments during truth-check: (1) CSP verification sentence rewritten (policy is a meta declaration, not a response header; page now states delivery mechanism honestly + upgrade commitment); (2) KTH sentence reworded to drop "independently evidenced". All 7 truth checks PASSED against production before build (CSP no unsafe-inline/eval, HTTPS 301, 429+Retry-After, security.txt valid Expires, form no-special-category statement, only __cf_bm cookie on fresh visit, check subdomain zero analytics requests). Body verified byte-identical to spec (link-stripped), no em/en dashes. Meta description 158 chars (spec claimed 157 — reported). Plumbing: TOP_PAGES entry in inject-writing-meta.js (flat security.html +body, hardened CSP, all 7 anchors prerendered), footer Legal column "Security" link (verified on /, /legal/privacy, essay route), llms.txt Core pages line, sitemap /security lastmod 2026-07-08. Privacy page related-links skipped (no such component exists, per spec). dateModified 2026-07-08. Article + BreadcrumbList JSON-LD. No other routes changed (git-verified). AWAITING DEPLOY → prod verification + RRT.

## Iteration 31b — 2026-07-08 (/security production verification — ALL LIVE)
Prod verified: title, 158-char description, canonical, og:type article + og:locale en_GB, H1, all 7 anchors in BOTH prerendered flat shell and live DOM, full body embedded in flat HTML, hardened CSP on the shell, all 6 in-body links, cite-block rendered, Article + BreadcrumbList JSON-LD parse clean, footer Security link live, sitemap lastmod 2026-07-08 + llms.txt line live. Note: footer links are React-rendered so they do not appear in flat shells (same as all other footer links — expected). User to run Google RRT on https://thirdrailsystems.ee/security.

## Iteration 31c — 2026-07-08 (/security RRT PASS + font-swap CSP fix)
Google RRT on /security: 3 valid items (Article, BreadcrumbList, Organization), crawl successful, eligible for rich results, zero errors. RRT console revealed the hardened CSP was blocking the inline onload="this.media='all'" Google Fonts swap (link stuck at media=print; Chrome still loaded the faces but cross-browser risk). Fix: removed inline onload from index.html; swap now performed CSP-safely in /js/error-guard.js (link.sheet check + load listener). Verified in local prod build: media flips to "all", Inter loads, renders clean. Second blocked inline script is Cloudflare edge-injected (bot management) — benign, outside our build. AWAITING DEPLOY for the font-swap fix.
